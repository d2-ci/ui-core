const points = (() => {
  ...
  return verticalPositions.reduce((all, vertical) => horizontalPositions.reduce((all, horizontal) => {
    all.push({
      vertical,
      horizontal
    });
...
const combinations = (() => points.reduce((all, anchorPoint) => points.reduce((all, popPoint) => {
  all.push([anchorPoint, popPoint]);
  return all;
}, all), []))();

const positions = (() => combinations.reduce((all, position) => {
