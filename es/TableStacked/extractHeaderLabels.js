...
const extractRowsFromTableChildren = children => React.Children.toArray(children) // extract theads
.filter(child => child.type === TableHead) // flatten rows
.map(({
  props
}) => props.children).reduce((flattened, row) => [...flattened, ...row], []) // map row to array of cell components
.map(({
  props
}) => props.children);

const calculateColumnCount = row => row.reduce((total, col) => col.props.colSpan ? total + parseInt(col.props.colSpan, 10) : total + 1, 0);
...
const extractLabelFromCell = cell => !cell.props.hideResponsiveLabel ? cell.props.label : '';
