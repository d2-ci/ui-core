export const getElementInnerDimension = (element, direction) => {
  ...
  if (element === document.body) {
    return Math.max(document.documentElement[`client${attribute}`], document.body[`scroll${attribute}`], document.documentElement[`scroll${attribute}`], document.body[`offset${attribute}`], document.documentElement[`offset${attribute}`]);
  }
