...
export function animatedScrollTo({
  ...
  const endValue = getEndValue(to, direction, scrollBox, offset, isWindowScroll, startValue);
  const change = endValue - startValue; // Just scroll into place if duration is 0
...
function getScrollHandler(scrollBox, direction, isWindowScroll) {
  switch (true) {
    ...
    case isWindowScroll && direction === HORIZONTAL:
      return scrollValue => window.scrollTo({
        left: scrollValue
      });
...
function getElemEndValue(el, direction, scrollBox, offset, isWindowScroll, startValue) {
  ...
  const movementSelector = direction === HORIZONTAL && el.offsetLeft > startValue || direction === VERTICAL && el.offsetTop > startValue ? 'forward' : 'back';
  const lookup = {
    window: {
      horizontal: {
